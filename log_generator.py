...
if __name__ == '__main__':
    ...
    #Add argument for number of log lines to generate
    ...
    #Add argument for wait time
    parser.add_argument("--sleep", "-s", help="Sleep this long between lines (in seconds)", default=0.0, type=float)

    #Add argument for ip address of ZeroMQ server
    parser.add_argument("--zeromq-server-ip", "-zmqsip", dest='zeromq_server_ip', help="IP address of ZeroMQ server", type=str)
    ...
    zeromq_server_ip = args.zeromq_server_ip
    ...
    #ZeroMQ client definition and configuration
    context = zmq.Context()
    socket = context.socket(zmq.REQ)
    socket.connect("tcp://" + zeromq_server_ip + ":5050")

    #Initiate log message creation
    flag = True
    while (flag):
        if args.sleep:
